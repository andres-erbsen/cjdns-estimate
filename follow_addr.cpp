...
uint query(uint u, uint target) {
	...
	while (u != target) {
		...
		for ( it=remotes[u].begin(); it != remotes[u].end(); ++it ) {
			...
			Q.push(Dijk(v, measure(target,v), weight ));
		...
		while (!Q.empty()) {
			...
			for ( it=remotes[v].begin(); it != remotes[v].end(); ++it ) {
				...
				if ( measure(target,w) <= measure(target,v) && D.find(w) == D.end()) {
					Q.push(Dijk(w, measure(target,w), so_far.aux + weight )); // search response
				}
			}
		}
	...
	while (1) {
		...
		for (uint i=0; i<1000; i++) {
			...
			if (cjdnspath == UINT_MAX) failed++;
				...
				if(cjdnspath < shortestpath[u][v]) printf("%d %d %d %d\n", u,v,cjdnspath,shortestpath[u][v]);
